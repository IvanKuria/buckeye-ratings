...
export function selectBestRmpMatch(
  ...
  // Ambiguity tiebreaker: when several candidates share a last name (and, on
  // initials-only portals, a first initial) they can match the search variants
  // with identical fuzzy scores. Fuse.js sorts stably, so pre-sorting candidates
  // by numRatings (desc) makes equal-score ties resolve to the most-reviewed
  // professor — the safest guess when the name alone can't disambiguate.
